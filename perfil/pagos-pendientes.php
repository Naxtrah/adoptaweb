...
try {
    ...
    foreach ($pendientes as &$pago) {
        ...
        //Si el monto almacenado difiere del cálculo actual, actualizar en base de datos
        if ($pago['monto'] != $total_vacunas) {
            ...
            $pago['monto'] = $total_vacunas; // Actualizar en array local también
        }
    }
    unset($pago);
                ...
                <div class="card-body">
                    ...
                    <?php if ($numPendientes > 0): ?>
                        ...
                        <div class="alert alert-success mb-4">
                            ...
                            Tienes <strong><?= $numPendientes ?></strong> pago(s) pendiente(s)
                        </div>
...
$stmt = $pdo->prepare("
    ...
    WHERE ap.id_usuario = ? 
      ...
      AND a.estado = 'Aprobada'   -- Solo adopciones que han sido aprobadas
    ORDER BY ap.id DESC
");
...
foreach ($filas as $i => $p) {
    //Calcular monto real de vacunas
    ...
    //Actualizar monto en BD si difiere del cálculo
    ...
    //Construir fila HTML para la tabla
    $html .= '
    <tr>
        <td>' . ($i + 1) . '</td>
        ...
        <td>
            <!-- Enlace para proceder al pago de vacunas -->
...
<?= $html ?>
                ...
                <!--Pie de tarjeta con información técnica-->
                <div class="card-footer bg-light">
                    <details>
                        ...
                        <div class="mt-3">
                            <div class="row">
                                <div class="col-md-6">
                                    <p class="mb-1 small"><strong>Usuario ID:</strong> <?= $user_id ?></p>
                                </div>
                                <div class="col-md-6">
                                    <p class="mb-1 small"><strong>Mostrados:</strong> <?= $numPendientes ?></p>
                                </div>
                            </div>
                        </div>
                    </details>
