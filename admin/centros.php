...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-10 p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2><i class="fas fa-home me-2"></i>Gestionar Centros</h2>
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modalCentro"><i class="fas fa-plus me-2"></i>Agregar Centro</button>
            </div>
            <table class="table table-bordered">
                <thead class="table-light"><tr><th>Imagen</th><th>Nombre</th><th>Dirección</th><th>Teléfono</th><th>Acciones</th></tr></thead>
                <tbody>
                    <?php foreach ($centros as $c): ?>
                    <tr>
                        <td>
                            <?php if (!empty($c['imagen_url'])): ?>
                                <img src="../<?= ltrim($c['imagen_url'], './') ?>" alt="<?= htmlspecialchars($c['nombre']) ?>" style="width: 60px; height: 60px; object-fit: cover;">
                            <?php else: ?>
                                <i class="fas fa-home fa-2x text-success"></i>
                            <?php endif; ?>
                        </td>
                        ...
                        <td><?= htmlspecialchars($c['telefono']) ?></td>
                        <td>
                            <a href="?accion=editar&id=<?= $c['id_centro'] ?>" class="btn btn-sm btn-warning">Editar</a>
                            <a href="?accion=eliminar&id=<?= $c['id_centro'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Eliminar?')">Eliminar</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <div class="modal-content">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <input type="hidden" name="id_centro" value="<?= $centro_editar['id_centro'] ?? 0 ?>">
                    <div class="row">
                        <div class="col-md-6 mb-3"><label class="form-label">Nombre *</label><input type="text" class="form-control" name="nombre" value="<?= $centro_editar['nombre'] ?? '' ?>" required></div>
                        <div class="col-md-6 mb-3"><label class="form-label">Teléfono</label><input type="text" class="form-control" name="telefono" value="<?= $centro_editar['telefono'] ?? '' ?>"></div>
                    </div>
                    <div class="mb-3"><label class="form-label">Dirección</label><input type="text" class="form-control" name="direccion" value="<?= $centro_editar['direccion'] ?? '' ?>"></div>
                    <div class="row">
                        <div class="col-md-6 mb-3"><label class="form-label">Email</label><input type="email" class="form-control" name="email" value="<?= $centro_editar['email'] ?? '' ?>"></div>
                        <div class="col-md-6 mb-3"><label class="form-label">Web</label><input type="text" class="form-control" name="web" value="<?= $centro_editar['web'] ?? '' ?>"></div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3"><label class="form-label">Latitud</label><input type="text" class="form-control" name="latitud" value="<?= $centro_editar['latitud'] ?? '' ?>"></div>
                        <div class="col-md-6 mb-3"><label class="form-label">Longitud</label><input type="text" class="form-control" name="longitud" value="<?= $centro_editar['longitud'] ?? '' ?>"></div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Imagen</label>
                        <input type="file" class="form-control" name="imagen" accept="image/*">
                        <?php if (!empty($centro_editar['imagen_url'])): ?>
                            <input type="hidden" name="imagen_actual" value="<?= $centro_editar['imagen_url'] ?>">
                            <img src="../<?= ltrim($centro_editar['imagen_url'], './') ?>" class="img-thumbnail mt-2" width="150">
                        <?php endif; ?>
                    </div>
                </div>
                <div class="modal-footer">
